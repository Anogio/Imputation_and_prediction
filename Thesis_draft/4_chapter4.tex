...
\emph{Remark:} It is clear (given that the loss is nonnegative) that $A+B+\sigma^2 \ge 0$, and standard inequalities show that $A$ (of the form $a^2 + b^2 - 2ab$) is nonnegative. However, nothing indicates that $B$ must be nonnegative. As we will see later (Chapter \ref{empirical}), in some cases the error may decrease linearly. However the next proposition shows that in good scenarios, this does not happen.
\end{proof}
