...
```{r}
splitted = train_test_split_MI(X,y,train_size,seed=seed)
spl = splitted$spl
...
preds_pool_harmonic = matrix(NA, nrow=length(y)-length(spl), ncol=n_imputations)
...
preds_pool_harmonic[,1] = y_pred_MI$y_pred[[1]][,2][-spl]
...
for(i in 2:n_imputations){
  ...
  preds_pool_harmonic[,i] = pool_MI_binary(preds=list(y_pred=y_pred_MI$y_pred[1:i]), spl=spl, method = 'harmonic')
...
Losses$harmonic = apply(preds_pool_harmonic, 2, f)
