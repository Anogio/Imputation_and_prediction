...
oneRun = function(args){
  ...
  #X.train = MCAR.nVars(X.train, miss_prop.train, 1)
  #X.test = MCAR.nVars(X.test, miss_prop.test, 1)
  datasets = list()
...
res.both = res %>% filter(miss_prop.test == miss_prop.train) %>% mutate(type='missInBoth', miss_prop=miss_prop.train)
res.stack = rbind(res.fullTest, res.fullTrain,res.both)
