...
oneRun = function(args){
  ...
  regressors.fit = lapply(datasets,
                          function(x) {pred.lin.train(x$train, y.train)})
  
  predictions = lapply(names(datasets), function(x){pred.lin.predict(regressors.fit[[x]], datasets[[x]]$test)})
