...
oneRun = function(args, missInTest=F){
  ...
  sigma = imp.withY.train$params$sigma
...
res.1 %>% gather('method','error', c(withY,withoutY, weighted.withY, weighted.withoutY, fullJoint)) %>% ggplot() + aes(x=p,y=error, color=method) + geom_line() + scale_y_log10()
```
...
res.3 %>% gather('method','error', c(withY,withoutY, weighted.withY, weighted.withoutY, fullJoint)) %>% ggplot() + aes(x=n,y=error, color=method) + geom_line() + scale_y_log10()
```
...
res.4 %>% gather('method','error', c(withY,withoutY, weighted.withY, weighted.withoutY, fullJoint)) %>% ggplot() + aes(x=miss_prop,y=error, color=method) + geom_line() + scale_y_log10()
```
