...
oneRun = function(args){
  ...
  pre.train = prelim.norm(X.train) 
  pre.test = prelim.norm(X.test)
  ...
  for(i in 1:m){
    ...
    pre.BS = prelim.norm(X.train.BS)
    imp.train.BS[[i]] = em.norm(pre.BS)
  }
  
  X.train.single.imp = imp.mvnorm.estim(list(thetahat=imp.train),X.train)
  ...
  MI.quantiles = t(apply(predictions.MI,1,function(x)quantile(x, probs=c(alpha,1-alpha)))) %>% as.data.frame()
  SI.quantiles = t(apply(predictions.SI,1,function(x)quantile(x, probs=c(alpha,1-alpha)))) %>% as.data.frame()
  BS.quantiles = t(apply(predictions.BS,1,function(x)quantile(x, probs=c(alpha,1-alpha)))) %>% as.data.frame()
  SIBS.quantiles = t(apply(predictions.SIBS,1,function(x)quantile(x, probs=c(alpha,1-alpha)))) %>% as.data.frame()
...
res.1 %>% gather('method','error', c(MI, SI, BS, SIBS)) %>% ggplot() + aes(x=p,y=error, color=method) + geom_line()
```
