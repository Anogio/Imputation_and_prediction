...
oneRun = function(args){
  ...
  return(c(lapply(isCovered, mean), list(var_B=mean(MI.var_b), var_W=mean(MI.var_w))))
}
```
```{r}
X.gen = X.basic.MVN
...
res.0 %>% rename('Partial MI coverage'=SI, 'Full MI coverage'=MI) %>% gather('method','error', c('Partial MI coverage','Full MI coverage')) %>% 
  ggplot() + aes(x=n,y=error, color=method) + geom_line() + geom_line(aes(y=1-alpha, color='Nominal coverage'))
```
...
res.1 %>% rename('Partial MI coverage'=SI, 'Full MI coverage'=MI) %>% gather('method','error', c('Partial MI coverage','Full MI coverage')) %>% 
  ggplot() + aes(x=p,y=error, color=method) + geom_line() + geom_line(aes(y=1-alpha, color='Nominal coverage'))
```
```{r}
X.gen = X.basic.MVN
y.gen = y.regression
...
argsL = list(n=1000,
            ...
            sigma_reg = 0.2,
            train_prop=0.6,
            miss_prop=seq(0,0.6,0.05),
            alpha = 0.3,
            m=30
)

S = 5
res.15 = evaluate.S.run.multiArg(S,argsL, oneRun, do.parallel = T) 

res.15 %>% rename('Partial MI coverage'=SI, 'Full MI coverage'=MI) %>% gather('method','error', c('Partial MI coverage','Full MI coverage')) %>% ggplot() + aes(x=miss_prop,y=error, color=method) + geom_line() + geom_line(aes(y=1-alpha, color='Nominal coverage'))
```
