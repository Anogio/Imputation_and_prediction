...
filled.datasets[['mice.pmm']] = complete(mice(X, m=1, method='pmm', printFlag = F))
filled.datasets[['mice.norm']] = complete(mice(X, m=1, method='norm', printFlag = F))
filled.datasets[['mean']] = complete(mice(X, m=1, method='mean', printFlag = F))
...
plot_grid(plot.amelia, plot.MF, plot.complete, labels=c('amelia', 'MissForest', 'complete'), ncol=2)
