...
imputation_methods = c('mean', 'mipca', 'mice') #any method accepted by the *impute* method from prediction_methods.R
...
```{r}
#importances = varImp(trained_predictors[['mice']][[4]])
#imp = data.frame(importances$importance)
#colnames(imp) = c('MICE')
#imp$Mean = varImp(trained_predictors[['mean']][[1]])$importance$Overall
#imp$MF = varImp(trained_predictors[['MF']][[1]])$importance$Overall
...
```
```{r}
positive_misclassification_penalty = 10
