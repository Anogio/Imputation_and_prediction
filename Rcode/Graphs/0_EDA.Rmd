...
colnames(X) = c('Sex', 'Age', "BMI", 'Glasgow', 'Heart.rate', 'Pulse.pressure', 'Hb.level', 'Oxygen.pulse', 'Cristalloids.IV', 'Colloids.IV')
...
ggplot(dat) + aes(x=Glasgow, fill=y) + geom_bar(position='dodge', aes(y =..prop..)) + 
  ...
  xlab('Initial Glasgow score') + ylab('Proportion of occurences') + 
...
```{r}
dat %>% gather('variable','value', c('Age','BMI','Heart.rate','Pulse.pressure', 'Hb.level', 'Oxygen.pulse')) %>%
  ggplot() + aes(x=value, color=y) + geom_density() + facet_wrap(~variable, scales = 'free') + themer() + xlab('Density') + ylab('Value')
```
