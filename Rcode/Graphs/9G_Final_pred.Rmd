...
res %>% group_by(pos.w) %>% summarise_all(mean) %>% filter(pos.w>=1)%>% 
  ...
  gather('method','error',-c(pos.weight, c3, c4, c5,noBMI, allCols, allPos, withY, pos.w, allNeg, fullJoint, completeCases, mean))%>%
  ggplot() + aes(x=pos.w, y=error, color=method) + geom_line() + 
