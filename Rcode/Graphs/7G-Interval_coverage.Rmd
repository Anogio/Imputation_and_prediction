...
lm.predVar2 = function(X.new, X.fit, y.fit, m){
  preds = matrix(NA, nrow=nrow(X.new), ncol=m)
  for(i in 1:m){
    in.BS = base::sample(1:nrow(X.fit), nrow(X.fit), replace=T)
    X.fit.BS = X.train[in.BS,]
    fit.BS = (predictor$train)(X.fit.BS, y.fit)
    preds[,i] = (predictor$predict)(fit.BS, X.new)
  }
  return(apply(preds,1,var))
}
```
...
res.0 %>% rename('Partial MI coverage'=SI, 'Full MI coverage'=MI) %>% gather('Legend','error', c('Partial MI coverage','Full MI coverage')) %>% 
  ggplot() + aes(x=sigma_reg,y=error, color=Legend) + geom_line() + geom_line(aes(y=1-alpha, color='Nominal coverage')) + 
  themer() +
  xlab('Train sample size') + ylab('Average coverage rate') + theme(legend.title = element_blank()) + 
