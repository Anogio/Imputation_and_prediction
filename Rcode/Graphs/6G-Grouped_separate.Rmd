...
```{r}
res.1 %>% rename('Grouped'=grouped, 'Separate'=separate, 'Identical'=correct, 'Full data'=full) %>%
...
```{r}
res.3 %>% rename('Grouped'=grouped, 'Separate'=separate, 'Identical'=correct, 'Full data'=full) %>%
  gather('method','error',c(Grouped, Separate, Identical,'Full data')) %>%
  ggplot() + aes(x=n*train_prop, y=error, color=method) + geom_line() +
  themer(angle=F) +
  scale_color_manual(values=c('black','red','forestgreen','darkblue'), name = "Imputation") +
