...
multiple_prediction <- function(X_MI, y, pred_method, train_size=0.5, seed=42, spl=NULL, cat.response=T, ...){
  ...
  for(i in 1:length(X_MI)){
    ...
    if(pred_method != 'logit'){
      ...
      fittedM = glm(y_train ~ .,family=binomial(link='logit'),data=dat_train)
      p = predict(fittedM, dat_full, type='response')
      y_pred[[i]] = cbind(1-p,p)
      trained_predictors[[i]] = fittedM
    }
  }
  print('Done.')
...
pool_MI_binary <- function(preds, spl=NULL, method='mean', y_true=NULL, quantile.split=0.5){
